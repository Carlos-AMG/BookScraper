...
async def search_book_task(
    ...
    query = Book_Query_Parameters(q=title, author=author, ext="pdf,epub")
...
@task(
    ...
    retry_delay_seconds=exponential_backoff(backoff_factor=2),
    retry_jitter_factor=0.5,
    name="get_download_links_member",
    cache_policy=NO_CACHE
)
async def get_member_download_links_task(
    annas_api: Annas_API,
    member_key: str,
    book_md5: str
) -> list[str]:
    ...
    logger = get_run_logger()
    annas_api.set_logger(logger)
    return await annas_api.member_download(book_md5, member_key)


@task(name="download_file", cache_policy=NO_CACHE)
...
async def download_single_book_flow(
    ...
    async with create_api_client(api_key, api_url) as (annas_api, session):
        ...
        try:
            download_links: list[str] = []
            if member_key:
                payload = await get_member_download_links_task(annas_api, member_key, matched_book.md5)
                fast = payload.get("fast")
                if isinstance(fast, str):
                    download_links.append(fast)
                basic = payload.get("basic")
                if isinstance(basic, list):
                    download_links.extend(basic)
            else:
                download_links = await get_download_links_task(annas_api, matched_book.md5)
            
            logger.info(f"Download links are: {download_links}")
        except Exception as e:
